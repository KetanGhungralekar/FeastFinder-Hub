...
//     @InjectMocks
//     private OrderServiceimpl orderService; // Replace with the actual implementation class
...
//         // Mock behaviors with exception handling
//         try {
//             // Mocking normal behavior for `getRestaurant`
//             restaurant.setId(orderReq.getRestaurantid());
//             when(restaurantService.getRestaurant(orderReq.getRestaurantid())).thenReturn(restaurant);
//             cart.setId(user.getId());
//             // Populate the cart with dummy cart items as needed
//             when(cartService.getCartByUser(user.getId())).thenReturn(cart);
        
//             // Call the method
//             Order order = orderService.CreateOrderItem(orderReq, user);
...
//         // Act: Call the method
//         Order result = orderService.CreateOrderItem(orderReq, user);
...
//             when(restaurantService.getRestaurant(orderReq.getRestaurantid())).thenReturn(restaurant);
//             when(cartService.getCartByUser(user.getId())).thenReturn(cart);
//             Order result1 = orderService.CreateOrderItem(orderReq, user);

//             // Assert: Verify results
//             assertNotNull(result1);
//             assertEquals(restaurant, result1.getRestaurant());
//             assertEquals(cart.getTotalPrice(), result1.getTotalPrice());
